...
// [Security Checker] Only Admin/Owner
...
// [OOP: Method Call]
// Menghitung total data (dengan filter pencarian jika ada) untuk paginasi
$total_data = $db->get_total_penghuni_filtered($cari);
$total_halaman = $total_data > 0 ? (int)ceil($total_data / $batas) : 1;

// [OOP: Method Call]
// Mengambil data penghuni yang sudah difilter dan dilimit
...
<head>
    ...
    <style>
        /* [UI/UX] Custom Style untuk jQuery Autocomplete agar lebih modern */
        .ui-autocomplete {
            background: white;
...
<body class="dashboard-body">
    ...
    <main class="main-content animate-fade-up">
        ...
        <div class="card-white">
                    ...
                    <tbody>
                        ...
                        if ($res->num_rows > 0) {
                            while ($row = $res->fetch_assoc()) {
                                // [Logic] Conditional Badge Status
                                ...
                                // [Logic] Format No HP untuk link WhatsApp (ganti 08 dgn 628)
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="flex flex-wrap gap-2">
                                            ...
                                            <!-- HANYA MUNCUL JIKA STATUS MASIH AKTIF -->
                                            <?php if ($row['status'] == 'AKTIF'): ?>
                                                <!-- 3. PERPANJANG (Membuka Modal) -->
            ...
            <script>
                // [UI Logic] Membuka Modal dan set value dinamis
            ...
            <div class="pagination-container" style="margin-top: 20px; display:flex; gap:5px; justify-content:center;">
                <?php
                // HTTP Query Builder: Agar keyword pencarian tidak hilang saat ganti halaman
                $qs = $_GET;
                $qs['halaman'] = $prev;
    ...
    <script>
        $(function() {
            // [AJAX] Autocomplete fitur pencarian penghuni
            var ac = $("#cari_penghuni").autocomplete({
                source: "../api/cari_penghuni.php", // Endpoint API
                minLength: 2, // Minimal 2 huruf baru trigger search
                select: function(event, ui) {
                    $(this).val(ui.item.value);
                    $(this).closest("form").submit(); // Auto submit form saat item dipilih
                }
            });

            // Custom UI render untuk highlight text yang dicocokkan
            ac.autocomplete("instance")._renderItem = function(ul, item) {
                var term = this.term;
                var re = new RegExp("(" + term + ")", "gi");
                // Menebalkan text yang cocok dengan keyword
